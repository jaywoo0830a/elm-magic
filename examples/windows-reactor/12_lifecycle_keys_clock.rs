...
//! | `on_key(..)` | **돌지 않는다** | 핸들러는 마지막 프레임이 `Ctx::keys`에 남긴 것이다. 호스트가 아레나(`&mut Ctx`)를 만질 수 없으므로 앱에서는 **WinUI `KeyAccelerators`**로 잡고 호스트 상태 → props로 표현한다(예제 05와 같은 방식). 0.100.0의 `AcceleratorKey`에는 `R`·`Enter`·사칙연산·숫자패드만 있다 — **`Ctrl+S`는 없다** |
//!
...
//! elm에는 **값만** 내려보낸다. elm 쪽 `on_tick`/`on_key`는 헤드리스 계약으로 남겨
//! 두면(테스트가 지킨다) 화면 코드는 플랫폼을 모른 채 그대로 재사용된다(예제 20).
...
use windows_reactor::{
    ...
    KeyAccelerator, KeyAccelerators, View, ViewContext, WindowVisuals,
};
...
enum RootMessage {
    /// 호스트 티커 — 자기 재무장.
    Tick,
    /// `Ctrl+R` (WinUI 가속기) — 결과는 props로 elm에 내려간다.
    ToggleSaved,
}
...
impl Component for Root {
    ...
    fn update(&mut self, message: RootMessage, context: &ComponentContext<Self>) {
        match message {
            ...
            RootMessage::ToggleSaved => self.saved = !self.saved,
        }
    }
    ...
    fn view(&self, _input: &(), context: &mut ViewContext<Self>) -> View {
        ...
        // 단축키는 WinUI가 잡는다 — `Ctx::keys`는 호스트가 부를 수 없다.
        // (`AcceleratorKey`에는 문자 키가 `R` 하나뿐이다.)
        let accelerators = KeyAccelerators::new([KeyAccelerator::new(
            AcceleratorKey::R,
            AcceleratorModifiers::Control,
            context.message(RootMessage::ToggleSaved),
        )]);

        Grid::new().key_accelerators(accelerators).children([View::fragment((
            ...
            format!("Ctrl+R saved: {}", self.saved),
        ))])
    }
}
