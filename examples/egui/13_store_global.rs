...
//!
//! **주의 — "전역"의 범위**: store는 **아레나마다 하나**다(`Arena.stores`).
//! 같은 컴포넌트 트리(한 `Ctx`) 안의 모든 컴포넌트가 공유하지만, **별개의
//! `ElmView`/`Ctx`끼리는 공유하지 않는다** — 화면 둘을 따로 마운트하면 store도
//! 두 벌이다. 화면 사이 공유가 필요하면 (1) 하나의 루트 컴포넌트로 묶거나
//! (2) 플랫폼 쪽 상태를 props로 내려보낸다.
//! 덕분에 **테스트끼리는 격리된다**(테스트마다 새 아레나).
